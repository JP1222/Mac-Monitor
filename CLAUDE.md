...
### `Process` (NSTask) has three foot-guns
- `task.terminationStatus` before the process exits throws `NSInvalidArgumentException`. Because `waitUntilExit()` is unbounded (next bullet) we poll instead of blocking, so the child may still be alive when we build the result — guard the read: `task.isRunning ? -1 : task.terminationStatus`. ([Apple docs](https://developer.apple.com/documentation/foundation/process/1415801-terminationstatus))
- If the child writes >~64KB to a pipe, calling `waitUntilExit()` before draining it **deadlocks** — and this includes **stderr even if you discard it**: an undrained stderr pipe fills and blocks the child just like stdout. Drain BOTH concurrently (background `readDataToEndOfFile`). Both `runShell` impls (`DeviceHealthCollector`, `AgentActions`) had this; fixed in `2511969`. `docker buildx prune` / `system df -v` easily exceed 64KB. ([Swift Forums — frozen Process](https://forums.swift.org/t/the-problem-with-a-frozen-process-in-swift-process-class/39579))
- **`waitUntilExit()` has no timeout** — so a child that ignores/blocks `SIGTERM` (a trap handler) or is stuck in uninterruptible state (a hung docker socket, `du` on a wedged mount) hangs it **forever**, pinning the agent's work-queue thread; enough of them exhaust the pool and `/health`/actions stop answering. We deliberately do **NOT** escalate to `SIGKILL` — force-killing `docker` orphans its children under launchd (observed `/health` degradation 2.5s → 11s → 20s), and SIGKILL doesn't rescue a true uninterruptible child anyway. Instead: after `SIGTERM`, **poll `task.isRunning` with a hard cap** rather than blocking, and bound the pipe drain the same way (`readGroup.wait(timeout:)`, or skip the synchronous drain while `task.isRunning` — its fds are still open and would re-block). Both `runShell` impls do this.
